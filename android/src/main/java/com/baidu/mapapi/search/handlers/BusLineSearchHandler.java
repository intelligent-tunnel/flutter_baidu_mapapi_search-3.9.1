...
public class BusLineSearchHandler extends MethodChannelHandler
    ...
    public void handleMethodCallResult(MethodCall call, MethodChannel.Result result) {
        ...
        busLineSearchOption.uid(buslineOptionBean.busLineUid.trim());
        if (!TextUtils.isEmpty(buslineOptionBean.startUid)) {
            busLineSearchOption.startUid(buslineOptionBean.startUid.trim());
        }
        if (!TextUtils.isEmpty(buslineOptionBean.endUid)) {
            busLineSearchOption.endUid(buslineOptionBean.endUid.trim());
        }
        LanguageType languageType = LanguageType.values()[buslineOptionBean.languageType];
        busLineSearchOption.language(languageType);
